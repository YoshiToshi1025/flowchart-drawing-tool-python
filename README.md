...
- フローチャートJSONファイルのExcel読込VBA(Widnows版)の未対応内容への対応（補足2を参照） 
...
* 現状のExcel読込VBAでは、以下、一部未対応の内容があります。Excel取り込み後に手動で調整願います。<br/>
　　- リンクの回り込み手動調整が反映されない。（対応検討中）<br/>
　　- 直線リンクの要素接続位置が、要素感最短部分でなく、図形のコネクター部分になる。（制限事項）<br/>
## 補足3
* Excel読込VBAはExcelVBAでインポートするだけで利用可能です。(追加のVBAプログラムや参照設定は不要)<br/>
　　- VBAインポート後、マクロ画面で DrawFlowchat を実行→ファイルダイアログでJSONファイルを選択→Excelアクティブシートに自動描画 の流れです。<br/>
* VBAインストール方法の詳細は、マニュアルに記載する予定です。（マニュアル未編集です_o_）