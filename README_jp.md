...
安易に描画できて業務的にも使えそうなツールが見あたらず、フローチャートの作図が面倒。
  - 設計書に残すために、フローチャートをExcelやPowerPointで作図しているけど、満足した形に整えるのに時間がかかり、ロジック修正や仕様追加による再編集が手間でうんざり。
  - 生成AIを活用する時代になって、文章だらけの仕様書になってしまってとても分かりにくい。生成AIにフローチャートを書かせてもイマイチで微修正もできない。

- そこで、「気軽に使えるフローチャート作図ツール」を目指して、Pythonでツールを作成しました。
- 簡易的なフローチャートを描くための基本的な機能が揃ったので、公開します。

## 本ツールの特徴
- 簡易フローチャートを短時間で作図、保存、再編集に対応
- 要素間のリンク接続および要素移動時にリンクを自動配線、手動操作での配線調整も可能
- 生成AI(GPT5.2)と連携して、フローチャートの自動作成に対応（自動作成後にフローチャートの手動編集可能）
